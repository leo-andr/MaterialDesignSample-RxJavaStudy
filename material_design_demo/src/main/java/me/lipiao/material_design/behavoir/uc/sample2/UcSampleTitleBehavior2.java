 ...
 * todo 该布局由RecylerView列表和一个TextView组成，其中RecylerView实现了NestedScrollingChild接口，
 * todo 所以TextView监听RecylerView的滑动状态。开始向上滑动列表时TextView和列表整体上移，直到TextView全部隐藏停止，
 * todo 再次上滑则列表内容上移。之后连续下滑列表当其第一个item全部显示时列表滑动停止，再次下滑列表时TextView跟随列表整体下移，
 * todo 直到TextView全部显示
